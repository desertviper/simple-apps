...
export class ToDoItemService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/to-do-items");
  ...
  create(toDoItem: IToDoItem): Observable<EntityResponseType> {
    return this.http.post<IToDoItem>(this.resourceUrl, toDoItem, {
      observe: "response",
    });
  }

  update(toDoItem: IToDoItem): Observable<EntityResponseType> {
    return this.http.put<IToDoItem>(
      `${this.resourceUrl}/${getToDoItemIdentifier(toDoItem) as number}`,
      toDoItem,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(toDoItem: IToDoItem): Observable<EntityResponseType> {
    return this.http.patch<IToDoItem>(
      `${this.resourceUrl}/${getToDoItemIdentifier(toDoItem) as number}`,
      toDoItem,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IToDoItem>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IToDoItem[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addToDoItemToCollectionIfMissing(
    toDoItemCollection: IToDoItem[],
    ...toDoItemsToCheck: (IToDoItem | null | undefined)[]
  ): IToDoItem[] {
    const toDoItems: IToDoItem[] = toDoItemsToCheck.filter(isPresent);
    if (toDoItems.length > 0) {
      const toDoItemCollectionIdentifiers = toDoItemCollection.map(
        (toDoItemItem) => getToDoItemIdentifier(toDoItemItem)!
      );
      const toDoItemsToAdd = toDoItems.filter((toDoItemItem) => {
        const toDoItemIdentifier = getToDoItemIdentifier(toDoItemItem);
        if (
          toDoItemIdentifier == null ||
          toDoItemCollectionIdentifiers.includes(toDoItemIdentifier)
        ) {
          return false;
        }
